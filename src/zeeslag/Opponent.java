...
//it thinks of the last ship that was hit and fills it in for all hit ships (it looks for the wrong ship that in fact already sank)
//at least it never forgets the first hit ship OR THE AIRCRAFT CARRIER
//intelligence necessities: fire at 1 around X if ship is not sunk. New: From two X on same ship, follow direction

//THIS IS WHAT AI IS SUPPOSED TO DO (ABOUT THE PROBLEM): LOOK FOR LAST HIT (X) IN MEMORY, CHECK IF THÁT SHIP HAS SUNK AND IF NOT, ATTACK
//major problem: the trigger to forget last ship is unknown, but it happens after the aircraft carrier was sunk
//you could see it as: after the aircraft carrier was sunk, it always goes random
	...
	public static void fire() throws Exception{
		...
		int test;
			...
			for(int i=99;i>=0;i--){				//it looks for the last hit on a still floating ship
				...
				if(memory[i][0]!=-1) System.out.println(i+" != -1 | "+memory[i][0]+","+memory[i][1]);															//THESE 3 LINES ARE TO BE DELETED WHEN AI OK
	...
	private static boolean isHitShipSunk(int memoryLocation){
		for(int i=0;i<Player.getAircraftCarrier().length;i++){				//below: check if ship hit was an aircraft carrier
			if(Player.getArray()[Player.getAircraftCarrier()[i][0]][Player.getAircraftCarrier()[i][1]] == 					//if location of point in ship on array == memory location on array
				...
				System.out.println("ship last hit at "+memoryLocation+" was an aircraft carrier");							//THIS LINE IS TO BE DELETED WHEN AI OK
				if(Player.isShipSunk(Player.getAircraftCarrier())){
					System.out.println("the aircraft carrier actually sank");												//THIS LINE IS TO BE DELETED WHEN AI OK (SAME FOR NEXT 4)
					return true;
				}
				else return false;
			}
